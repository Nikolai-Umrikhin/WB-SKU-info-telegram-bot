...
class DataBase
{
    ...
    {
        $this->query("INSERT INTO `telegram_users` (telegram_id, username, first_name, status) VALUES ( :telegram_id, :username, :first_name, :status )",
            $params);
    }

    public function QuerryStatusUpdate($params)
    {
        $this->query('UPDATE `telegram_users` SET `username` = :username, `first_name` = :first_name, `status`= :status WHERE `telegram_id` = :telegram_id',
            $params);
    }
